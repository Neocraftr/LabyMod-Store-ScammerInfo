...
public class TrustedCommandListener implements ClientCommandEvent {
    ...
    public boolean onCommand(String cmd, String[] args) {
                ...
                new Thread(() -> {
                    ...
                    if (uuid != null) {
                        ...
                        if (!sc.getListManager().getPrivateListTrusted().containsUUID(uuid)) {
                            ...
                            if(description != null) {
                                sc.displayMessage(ScammerList.PREFIX + "§aDer Spieler §e" + names.get(0) + " §awurde wegen §e"+description+" §azu deiner Trustedliste hinzugefügt.");
                            } else {
                                sc.displayMessage(ScammerList.PREFIX + "§aDer Spieler §e" + names.get(0) + " §awurde zu deiner Trustedliste hinzugefügt.");
                            }
                            sc.getListManager().getPrivateListTrusted().save();
                        } else {
                            sc.displayMessage(ScammerList.PREFIX + "§cDer Spieler §e" + names.get(0) + " §cbefindet sich bereits auf deiner Trustedliste.");
                        }
                    } else {
                ...
                new Thread(() -> {
                    ...
                    if (uuid != null) {
                        ...
                        if (sc.getListManager().getPrivateListTrusted().removeByUUID(uuid)) {
                            sc.displayMessage(ScammerList.PREFIX + "§aDer Spieler §e" + name + " §awurde von deiner Trustedliste entfernt.");
                            sc.getListManager().getPrivateListTrusted().save();
                        } else {
                            sc.displayMessage(ScammerList.PREFIX + "§cDer Spieler §e" + name + " §cbefindet sich nicht auf deiner Trustedliste.");
                        }
                    } else {
                        sc.displayMessage(ScammerList.PREFIX + "§cEs gibt keinen Spieler mit diesem Namen.");
                    }
                    ...
                    if (uuid != null) {
                        ...
                        if(sc.getListManager().checkUUID(uuid, PlayerType.TRUSTED)) {
                            ...
                            if(nameHistory.size() ==  1) {
                                joiner.add("§aName: §e"+nameHistory.get(0));
                            } else {
                                joiner.add("§aName: §e"+nameHistory.get(0)+" ["+nameHistory.get(1)+"]");
                            }
                            joiner.add("§aUUID: §e"+(uuid.equals(nameHistory.get(0)) ? "Nicht verfügbar" : uuid));

                            List<String> containingLists = sc.getListManager().getContainingLists(uuid, PlayerType.TRUSTED);
                            joiner.add("§aListe: §e"+formatList(containingLists));
                            if(containingLists.contains("Privat")) {
                                Scammer s = sc.getListManager().getPrivateListTrusted().getByUUID(uuid);
                                if(s.getDate() != 0) joiner.add("§aHinzugefügt am: §e"+new SimpleDateFormat("dd:MM:yyyy HH:mm").format(new Date(s.getDate())));
                                if(s.getOriginalName() != null) joiner.add("§aUrsprünglicher Name: §e"+s.getOriginalName());
                                if(s.getDescription() != null) joiner.add("§aBeschreibung: §e"+s.getDescription());
                            }

                            joiner.add(ScammerList.PREFIX_LINE);
                            sc.displayMessage(joiner.toString());
                        } else {
                            sc.displayMessage(ScammerList.PREFIX + "§cDer Spieler §e" + nameHistory.get(0) + " §cbefindet sich nicht auf der Trustedliste.");
                        }
                    } else {
                        sc.displayMessage(ScammerList.PREFIX + "§cEs gibt keinen Spieler mit diesem Namen.");
                    }
       ...
       if(args[0].equalsIgnoreCase("checkall")) {
           ...
           if(scammers.size() == 0) {
               sc.displayMessage(ScammerList.PREFIX + "§cKeine Trusted Spieler auf diesem CityBuild...");
           } else {
               sc.displayMessage(ScammerList.PREFIX + "§eTrusted Spieler auf diesem CityBuild: §a" + String.join("§e, §a", scammers));
           }
       } else
        ...
        if (args[0].equalsIgnoreCase("list")) {
            ...
            if (listSize != 0) {
                ...
                sc.displayMessage(ScammerList.PREFIX + "§cDeine Trustedliste ist leer.");
            }
        } else
        ...
        if (args[0].equalsIgnoreCase("clear")) {
            if (args.length >= 2 && args[1].equalsIgnoreCase("confirm")) {
                if (confirmClear) {
                    ...
                    sc.getListManager().getPrivateListTrusted().save();
                    sc.displayMessage(ScammerList.PREFIX + "§aAlle Einträge deiner Trustedliste wurden gelöscht.");
                } else {
    ...
    private void printHelp() {
        ...
        joiner.add("§e"+ScammerList.COMMAND_PREFIX+"trusted add <Name> [Grund] §8- §aFügt einen Spieler zur Trustedliste hinzu.");
        joiner.add("§e"+ScammerList.COMMAND_PREFIX+"trusted remove <Name> §8- §aEntfernt einen Spieler von der Trustedliste.");
        joiner.add("§e"+ScammerList.COMMAND_PREFIX+"trusted addclan <Name|ClanTag §8- §aFügt die Spieler eines Clans zur Trustedliste hinzu.");
        joiner.add("§e"+ScammerList.COMMAND_PREFIX+"trusted removeclan <Name|ClanTag> §8- §aEntfernt die Spieler eines Clans von der Trustedliste.");
        joiner.add("§e"+ScammerList.COMMAND_PREFIX+"trusted check <Name> §8- §aÜberprüft ob sich ein Spieler auf der Trustedliste befindet.");
        joiner.add("§e"+ScammerList.COMMAND_PREFIX+"trusted checkall §8- §aZeigt alle Scammer auf dem CityBuild an.");
        joiner.add("§e"+ScammerList.COMMAND_PREFIX+"trusted clear §8- §aEntfernt alle Spieler von der Trustedliste.");
        joiner.add("§e"+ScammerList.COMMAND_PREFIX+"trusted list §8- §aZeigt alle Spieler auf der Trustedliste.");
        joiner.add("§e"+ScammerList.COMMAND_PREFIX+"trusted update §8- §aAktualisiert die Namen der Spieler. (Wird automatisch durchgeführt)");
        joiner.add("§e"+ScammerList.COMMAND_PREFIX+"trusted version §8- §aZeigt die Version des Addons an.");
        joiner.add(ScammerList.PREFIX_LINE);
        sc.displayMessage(joiner.toString());
    }
